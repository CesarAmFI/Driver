...
void Parser::rule_declaration_variable(int type, std::string id) {
    ...
    list.push_back(Expression(type, id));
...
void Parser::rule_declaration_structure(std::string id) {
    ...
    eat(ID);
    std::vector<Expression> list;
    list.push_back(Expression(type, list_id));
    rule_list_variables(type, list);
    eat(SEMICOLON);
}
...
void Parser::rule_statement_return_() {
    if (token == SEMICOLON) {
        eat(SEMICOLON);
        driver.generate_return_void();
    }
    else if (token == NEGATE || token == ID || token == LEFT_PARENTHESIS || token == NUMBER_INTEGER || token == NUMBER_FLOAT || token == NUMBER_DOUBLE) {
        Expression expression = rule_expression_a();
        eat(SEMICOLON);
        driver.semantic_return_expression(expression);
        driver.semantic_variables(driver.generate_return(expression));
    }
}
...
Expression Parser::rule_expression_a() {
    ...
    return Expression(expression_a_.get_type(), expression_a_.get_address());
}
...
Expression Parser::rule_expression_a_(Expression expression) {
    if (token == DISJOIN) {
        ...
        rule_expression_a_(expression_);
        return expression_;
    }
    return expression;
}
...
Expression Parser::rule_expression_b() {
    ...
    return Expression(expression_b_.get_type(), expression_b_.get_address());
}
...
Expression Parser::rule_expression_b_(Expression expression) {
    if (token == CONJOIN) {
        ...
        driver.semantic_variables(temporals);
        Expression expression_ = temporals[0];
        rule_expression_b_(expression_);
        return expression_;
    }
    return expression;
}
...
Expression Parser::rule_expression_c() {
    ...
    return Expression(expression_c_.get_type(), expression_c_.get_address());
}
...
Expression Parser::rule_expression_c_(Expression expression) {
    if (token == EQUAL) {
        ...
        driver.semantic_variables(temporals);
        Expression expression_ = temporals[0];
        rule_expression_c_(expression_);
        return expression_;
    }
...
Expression Parser::rule_expression_d() {
    ...
    return Expression(expression_d_.get_type(), expression_d_.get_address());
}
...
Expression Parser::rule_expression_d_(Expression expression) {
    if (token == LESS) {
        ...
        driver.semantic_variables(temporals);
        Expression expression_ = temporals[0];
        rule_expression_d_(expression_);
        return expression_;
    }
...
Expression Parser::rule_expression_e() {
    ...
    return Expression(expression_e_.get_type(), expression_e_.get_address());
}
...
Expression Parser::rule_expression_e_(Expression expression) {
    if (token == ADD) {
        ...
        driver.semantic_variables(temporals);
        Expression expression_ = temporals[0]; 
        rule_expression_e_(expression_);
        return expression_;
    }
...
Expression Parser::rule_expression_f() {
    ...
    return Expression(expression_f_.get_type(), expression_f_.get_address());
}
...
Expression Parser::rule_expression_f_(Expression expression) {
    if (token == MULTIPLY) {
        ...
        driver.semantic_variables(temporals);
        Expression expression_ = temporals[0];         
        rule_expression_f_(expression_);
        return expression_;
    }
...
Expression Parser::rule_expression_h() {
    if (token == ID) {
        ...
        float value = std::stoi(lexer->YYText());
        eat(NUMBER_FLOAT);
...
Expression Parser::rule_expression_h_(std::string id) {
    if (token == LEFT_PARENTHESIS) {
        ...
        std::vector<Expression> temporals = driver.generate_call(expression_function, expression_number);
        driver.semantic_variables(temporals);
        Expression expression = temporals[0];
        return expression;
    }
    else if (token == DOT) {
        eat(DOT);
...
std::vector<Expression> Parser::rule_parameters_(std::string id, std::vector<Expression> list) {
    if (token == COMMA) {
        ...
        for (int i = 0; i < list.size(); i++) driver.generate_parameter(list[i], driver.expression_from_argument(id, i));
        return list;
    }
}
