...
bool Driver::is_creatable(Expression expression) {
...
bool Driver::are_compatible(Expression expression_1, Expression expression_2) {
    Type type_1 = this->type_table.get_type_by_id(expression_1.get_type());
    Type type_2 = this->type_table.get_type_by_id(expression_2.get_type());
    if (is_creatable(expression_1) && is_creatable(expression_2)) return true;
    if (type_1.get_name() == "struct" && type_2.get_name() == "struct") return type_1.get_base_type().is_equivalent(type_2.get_base_type());
    return false;
}
...
Expression Driver::expression_from_label(int id) {
    return Expression(-1, "L" + std::to_string(id));
}

Expression Driver::expression_from_label(std::string id) {
    return Expression(-1, id);
}

Expression Driver::expression_from_return() {
    return Expression(this->type_return, "");
}

Expression Driver::expression_from_integer(int number) {
    return Expression(this->type_table.get_type_by_name("int").get_id(), std::to_string(number));
}

Expression Driver::expression_from_temporary(int id, int type) {
    return Expression(type, "t" + std::to_string(id));
}
...
Expression Driver::expression_from_function(std::string id) {
    ...
    return Expression(function.get_type(), id);
}
...
Expression Driver::expression_from_argument(std::string id, int position) {
    ...
    return Expression(arguments[position].get_type(), arguments[position].get_address());
}
...
Expression Driver::expression_from_structure(std::string id) {
    ...
    return Expression(structure.get_type(), id);
}
...
Expression Driver::expression_from_structure_variable(std::string id, std::string attribute) {
    ...
    Symbol var = this->symbol_table_stack.get_global_table().get_symbol_by_id(id);
    Symbol attr = this->type_table.get_type_by_id(var.get_type()).get_base_type().get_symbol_by_id(attribute);
    return Expression(attr.get_type(), id + "[" + std::to_string(attr.get_address()) + "]");
}
...
Expression Driver::expression_from_variable(std::string id) {
    ...
    return Expression(variable.get_type(), id);
}
...
Expression Driver::expression_from_constant(std::string id) {
    ...
    return Expression(constant.get_type(), constant.get_id());
}
...
void Driver::semantic_prepare_program() {
    ...
    this->type_table.add_type(Type(2, "char", 1, 0, SymbolTable()));
...
void Driver::semantic_function(std::string id, std::vector<Expression> arguments) {
    ...
    this->symbol_table_stack.get_top_table().add_symbol(Symbol("__RETURN__", this->type_return, this->address, "return", {}));
    this->address += this->type_table.get_type_by_id(this->type_return).get_size();
    this->symbol_table_stack.get_global_table().add_symbol(Symbol(id, this->type_return, -1, "fun", arguments));
    semantic_destroy_context();
}
...
void Driver::semantic_structure(std::string id) {
    ...
    int size = 0;
    for (Symbol symbol : base_type.get_symbols()) size += type_table.get_type_by_id(symbol.get_type()).get_size();
    this->type_table.add_type(Type(type_id, "struct", size, base_type.get_symbols().size(), base_type));
...
void Driver::semantic_variables(std::vector<Expression> variables) {
    for (Expression variable : variables) {
        std::string id = variable.get_address();
        int type = variable.get_type();
        if (exists_local_symbol(id)) error("Simbolo con id " + id + " ya existe");
        this->symbol_table_stack.get_top_table().add_symbol(Symbol(id, type, this->address, "var", {}));
        this->address += this->type_table.get_type_by_id(type).get_size();
    }
}

void Driver::semantic_arguments(std::vector<Expression> arguments) {
    for (Expression argument : arguments) {
        std::string id = argument.get_address();
        int type = argument.get_type();
        if (exists_local_symbol(id)) error("Simbolo con id " + id + " ya existe");
        Symbol symbol(id, type, this->address, "param", {});
        this->symbol_table_stack.get_top_table().add_symbol(symbol);
        this->address += this->type_table.get_type_by_id(type).get_size();
    }
}
...
std::vector<Expression> Driver::generate_operation(Expression expression_1, Expression expression_2, std::string operation) {
    ...
    Quad quad = Quad(expression.get_address(), expressions_1[0].get_address(), operation, expressions_2[0].get_address());
...
std::vector<Expression> Driver::generate_call(Expression expression_1, Expression expression_2) {
    int temporary = create_temporary();
    Expression expression = expression_from_temporary(temporary, expression_1.get_type());
    Quad quad = Quad(expression.get_address(), expression_1.get_address(), "call", expression_2.get_address());
    this->code_generator.push_back(quad);
...
std::vector<Expression> Driver::generate_negation(Expression expression) {
    ...
    Quad quad = Quad(expression_.get_address(), expression.get_address(), "!", "");
    this->code_generator.push_back(quad);
    return std::vector<Expression>{expression};
}
...
std::vector<Expression> Driver::generate_cast(Expression expression_1, Expression expression_2) {
    ...
    Quad quad = Quad(expression.get_address(), expression_1.get_address(), cast, "");
    this->code_generator.push_back(quad);
    return std::vector<Expression>{expression};
}

std::vector<Expression> Driver::generate_return(Expression expression) {
    Expression expression_return = expression_from_return();
    if (!are_compatible(expression, expression_return)) error("Tipo de retorno incompatible");
    bool will_cast = are_castable(expression, expression_return);
    std::vector<Expression> expressions_ = will_cast ? generate_cast(expression, expression_return) : std::vector<Expression>{expression};
    Quad quad = Quad("", expressions_[0].get_address(), "return", "");
    this->code_generator.push_back(quad);
    std::vector<Expression> expressions;
    if (will_cast) expressions.insert(expressions.end(), expressions_.begin(), expressions_.end());
    return expressions;
}

std::vector<Expression> Driver::generate_parameter(Expression expression_1, Expression expression_2) {
    if (!are_compatible(expression_1, expression_2)) error("Parametros incompatibles");
    bool will_cast = are_castable(expression_1, expression_2);
    std::vector<Expression> expressions_ = will_cast ? generate_cast(expression_1, expression_2) : std::vector<Expression>{expression_1};
    Quad quad = Quad("", expressions_[0].get_address(), "param", "");
    this->code_generator.push_back(quad);
    std::vector<Expression> expressions;
    if (will_cast) expressions.insert(expressions.end(), expressions_.begin(), expressions_.end());
    return expressions;
...
std::vector<Expression> Driver::generate_assignment(Expression expression_1, Expression expression_2) {
    ...
    Quad quad = Quad(expression_1.get_address(), expressions_[0].get_address(), "=", "");
    this->code_generator.push_back(quad);
    std::vector<Expression> expressions;
...
void Driver::generate_if(Expression expression_1, Expression expression_2) {
    Quad quad = Quad("", expression_1.get_address(), "if", expression_2.get_address());
    this->code_generator.push_back(quad);
}

void Driver::generate_label(Expression expression) {
    Quad quad = Quad("", expression.get_address(), "label", "");
    this->code_generator.push_back(quad);
}

void Driver::generate_goto(Expression expression) {
    Quad quad = Quad("", expression.get_address(), "goto", "");
    this->code_generator.push_back(quad);
}

void Driver::generate_print(Expression expression) {
    Quad quad = Quad("", expression.get_address(), "print", "");
    this->code_generator.push_back(quad);
}

void Driver::generate_scan(Expression expression) {
    Quad quad = Quad("", expression.get_address(), "scan", "");
    this->code_generator.push_back(quad);
}

void Driver::generate_return_void() {
    Quad quad = Quad("", "", "return void", "");
    this->code_generator.push_back(quad);
}
...
void Driver::complete_generate_return_void() {
    if (!this->has_return && this->type_return == this->type_table.get_type_by_name("void").get_id()) generate_return_void();
}
